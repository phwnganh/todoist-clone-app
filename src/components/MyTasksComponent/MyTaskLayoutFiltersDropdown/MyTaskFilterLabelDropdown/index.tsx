...
const MyTaskFilterLabelDropdown = ({selectedFilteringLabel, onSelectFilteringLabel}: MyTaskFilterLabelDropdownProps) => {
    ...
    const {data: filteredLabels} = useSearchLabels(debouncedSearchFilteringLabels)
    const {data: labels} = useGetAllLabels()
    const labelList = debouncedSearchFilteringLabels.length > 0 ? (filteredLabels?.results ?? []) : labels?.results
    ...
    return (
        <div id={"filtering-label-listbox"} aria-labelledby={"filtering-label-trigger"} role={"listbox"} className={"absolute top-full right-0 z-50 border border-product-library-divider-primary rounded-large shadow-sm max-w-214.75 w-full bg-white p-1.5 cursor-pointer"
        }>
                    <div className={"flex flex-col py-1 px-1.5 w-full"}>
                        <SearchingLabelInput labelValue={typedFilteredLabels} onLabelsSearched={setTypedFilteredLabels} inputRef={searchInputRef}/>
                        <hr className={"border-t border-t-product-library-divider-tertiary p-1.5"}/>
                        {isNoLabelsFound ? (<p className="text-sm text-product-library-display-secondary-idle-tint px-2.5 py-1">
                            Label not found
                        </p>) : (
                            labelList?.map(label => {

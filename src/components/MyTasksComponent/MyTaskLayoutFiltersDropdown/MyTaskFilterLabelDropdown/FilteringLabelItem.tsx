...
const FilteringLabelItem = ({label, isLabelsSelected, onFilteringLabelsSelected}: FilterLabelItemProps) => {
    return (
        <div className={"flex items-center gap-small py-1 px-1.5 hover:bg-product-library-selectable-secondary-hover-fill rounded-small"} role={"option"} tabIndex={-1}>
                <input type={"checkbox"} className={"w-4 h-4"} checked={isLabelsSelected} onChange={(e) => {
                    e.stopPropagation()

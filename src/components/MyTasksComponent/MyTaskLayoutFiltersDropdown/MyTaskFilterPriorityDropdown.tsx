...
const MyTaskFilterPriorityDropdown = () => {
    return (
        <div id={"priority-listbox"} aria-labelledby={"priority-trigger"} role={"listbox"} className={"absolute top-full right-0 z-50 border border-product-library-divider-primary rounded-large shadow-sm max-h-70 max-w-40 w-full bg-white p-1.5 cursor-pointer"}>
            {priorityFilterData.map(priority => {
                return (
                    <div key={priority.key} role={"option"} tabIndex={-1} className={"group flex flex-col py-1 px-1.5 flex-1 w-full hover:bg-product-library-selectable-secondary-hover-fill rounded-small"}>

...
const ChildrenTaskItem = ({
  ...
  subChildren,
    ...
    const {setNodeRef, attributes, listeners, transition, transform} = useSortable({id: childrenTask.id, data: {
        type: "subTask"
        }})
  ...
  return (
      <>
          {isEditing ? <EditMyTaskDetailMainSubChildrenForm onCloseEditMySubTask={onCloseEditTask} taskDetail={childrenTask} /> : (
              <div className={"flex items-start gap-3"} ref={setNodeRef} style={style}>
                  <button type={"button"} className={"flex justify-center w-6 h-6 shrink-0 hover:bg-product-library-selectable-secondary-hover-fill rounded-small"} {...attributes} {...listeners}>
                      <DragDropIcon/>
                  </button>
                  <div className={"flex justify-between items-start w-full lg:items-center group"}>
                      ...
                      <div className={"flex pl-4 gap-small opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto"}>
                          <button onClick={() => onOpenEditTask(childrenTask.id)} type={"button"} aria-label={"edit"} className={"w-6 h-6 rounded-small hover:bg-product-library-selectable-secondary-hover-fill"}>
                              <EditIcon/>
                          </button>
                          ...
                          <div className={"relative"}>
                              ...
                              {isOpenSubTaskDetailToolbar && <div className={"absolute right-9 z-50"}
                                  ...
                                  <MyTasksToolbarDropdown taskId={childrenTask.id} task={childrenTask}/>
                              </div>}
                          </div>

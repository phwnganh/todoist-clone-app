...
const MyTaskDetailHeaderForm = ({onCancelForm}: {onCancelForm: () => void}) => {
    return (
        <form className={"w-full"}>
            <div className={"outline-product-library-border-hover-tint rounded-large outline flex flex-col pt-1 px-1.75"}>
            <input type={"text"} className={"outline-none font-medium text-header"} placeholder={"Content"}/>
            <div className={"flex mt-small mb-xsmall ml-px"}>
                <div className={"flex justify-center items-center"}>
                    <img src={DescriptionIcon} alt={"description-icon"}/>
                </div>
                <input type={"text"} className={"outline-none text-sm text-product-library-display-secondary-idle-tint"} placeholder={"Description"}/>
            </div>
            </div>

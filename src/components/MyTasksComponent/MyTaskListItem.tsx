...
const MyTaskListItem = ({taskNode, level, onOpenTaskDetailToolbar, onCloseTaskDetailToolbar, isOpenTaskDetailToolbar}: MyTaskListItemProps) => {
    ...
    return (
        <>
            {isEditing ? (<EditMyTaskModalDialog variant={"list"} task={task} onCloseEditMyTask={onCloseEditTask}/>) : (
                <li className={`px-2 border-b border-b-product-library-divider-primary flex justify-between items-start group relative ${getTaskIndentClass(level)}`}>
                    ...
                    <div className={"group-hover:flex hidden mt-2 pl-4 gap-small"}>
                        ...
                        <div className={"relative"}>
                            <button type={"button"} onClick={onOpenTaskDetailToolbar} aria-label={"menu"} className={"rounded-small hover:bg-product-library-selectable-secondary-hover-fill"}>
                                <MenuIcon/>
                            </button>
                            {isOpenTaskDetailToolbar && (
                                <div className={"absolute right-9 z-50"} onClick={e => e.stopPropagation()}>
            ...
            {hasChildren && isExpanded && children.map(child => (<MyTaskListItem key={child.task.id} taskNode={child} level={level + 1} isOpenTaskDetailToolbar={isOpenTaskDetailToolbar} onOpenTaskDetailToolbar={e => {

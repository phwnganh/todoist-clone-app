...
const SidebarNavItem = ({item, onOpenSearchModalDialog}: SidebarNavItemProps) => {
    ...
    return (
        <NavLink to={item.getTo} className={({isActive}) => `${commonClass} hover:rounded-small ${isActive ? "bg-product-library-priorities-p2-secondary-hover-fill rounded-small hover:bg-product-library-priorities-p2-secondary-hover-fill" : "hover:bg-product-library-selectable-secondary-hover-fill"}`}>

...
const SidebarMyProjectsItem = ({project}: {project: Project}) => {
    return (
        <NavLink to={`${PROJECT_DETAILS}/${project.id}`} className={({isActive}) => `flex items-center rounded-small ${isActive ? "bg-product-library-priorities-p2-secondary-hover-fill hover:bg-product-library-priorities-p2-secondary-hover-fill" : "hover:bg-product-library-selectable-secondary-hover-fill"}`}>
            {({isActive}) => <>
                <div className={`p-1.25 flex items-center ${isActive ? "text-product-library-actionable-tertiary-idle-tint" : ""}`}>

...
export const sortTasks = (tasks: Task[], sortedBy?: SortedBy | null, order?: SortOrder): Task[] => {
    ...
    return [...tasks].sort((a, b) => {
        ...
        switch (sortedBy){
            case "ALPHABETICALLY":
                ...
                break;
            case "ADDED_DATE":
                res = new Date(a.added_at ?? 0).getTime() - new Date(b.added_at ?? 0).getTime()
                break;
            case "DUE_DATE":
                res = new Date(a.due?.date ?? 0).getTime() - new Date(b.due?.date ?? 0).getTime()
                break;
            case "PRIORITY":
...
export const groupTasks = (tasks: Task[], grouped_by?: GroupedBy): TaskGroup[] => {
    if(!grouped_by) return [
        {
            title: "",
            tasks
        }

...
import {
  useGetAllSections,
  useReorderSection,
} from "@/hooks/useQueryHook/useSections.ts";
...
import {
  findSectionByIdToOrder,
  handleReorderSection,
} from "@/helpers/dragDropMySection.ts";
import {
  useMoveMyTask,
  useReorderTask,
...
import {
  findTaskByIdToOrder,
  handleReorderTask,
} from "@/helpers/dragDropMyTasks.ts";
...
const InboxPage = () => {
  ...
  const handleDragEnd = async (e: DragEndEvent) => {
    ...
    const isDropInside =
      overType === "task" && activeType === "task" && dropType === "inside";
    if (activeType === "section" && overType === "section") {
      const sectionList = sections?.results;
      if (!sectionList) return;

      const activeSection = findSectionByIdToOrder(
        sectionList,
        active.id as string,
      );
      const overSection = findSectionByIdToOrder(
        sectionList,
        over.id as string,
      );
          ...
          <div className={"flex justify-end items-center"}>
            <div className={"relative p-1 md:p-2.5"}>
              <button
                type={"button"}
                className={
                  "flex items-center px-1.5 hover:bg-product-library-selectable-secondary-hover-fill rounded-small"
                }
                onClick={handleOpenLayoutDropdown}
              >
                <div
                  className={
                ...
                <span className="hidden md:block text-product-library-actionable-quaternary-idle-tint text-sm font-medium">
                  Display
                </span>
              </button>
              {openLayoutDropdown && (
                <MyTaskLayoutFiltersDropdown
                  onSelectLayout={handleSelectLayout}
                  layoutTitle={layoutName}
                  onUpdateViewOption={handleUpdateViewOption}
                  viewType={"PROJECT"}
                  viewId={inboxProjectId}
                />
              )}
            </div>
            <button
              type={"button"}
              className={
                "flex items-center justify-center w-9 h-9 shrink-0 p-1.5 hover:bg-product-library-selectable-secondary-hover-fill rounded-small"
              }
            >
              <CommentIcon
                className={
        ...
        {layoutName === "LIST" ? (
          ...
          <section className={"px-10"}>
            <div className={"flex flex-col gap-small"}>
              ...
              <MyTasksBoard
                filteredSectionsByProject={sections?.results}
                isGrouping={isGrouping}
                groupedTasks={groupedTasks}
                tasks={allTasks?.results ?? []}
                noSection={NO_SECTION}
